...
class TestPollingFallback:
    ...
    @patch('file_watcher.Observer', side_effect=ImportError)
    def test_polling_fallback_starts_when_watchdog_fails(self, mock_observer, temp_config_dir):
        """Test that polling fallback starts when watchdog Observer fails."""
        callback = Mock()

        # use_polling=False but Observer import fails, should fall back to polling
        watcher = start_file_watcher(str(temp_config_dir), callback, use_polling=False)
